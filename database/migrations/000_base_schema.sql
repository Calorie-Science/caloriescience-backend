-- Migration 000: Base database schema
-- This creates all the foundational tables, enums, indexes, triggers, and RLS policies
...
CREATE TABLE IF NOT EXISTS client_nutrition_requirements (
  id UUID DEFAULT uuid_generate_v4() PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_meal_plans_active ON meal_plans(client_id, is_active) WHERE is_active = true;
...
-- RLS Policies (with IF NOT EXISTS equivalent)
DO $$ BEGIN
...
-- Insert demo data (optional - only if no users exist)
INSERT INTO users (email, password_hash, full_name, qualification, experience_years) 
SELECT 'demo@caloriescience.com', '$2b$10$example', 'Demo Nutritionist', 'MSc Clinical Nutrition', 5
WHERE NOT EXISTS (SELECT 1 FROM users WHERE email = 'demo@caloriescience.com'); 